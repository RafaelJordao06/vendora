...
import { Check, Loader2, DollarSign, Calendar, TrendingUp, Trash2, RotateCcw } from "lucide-react"
...
export function PurchaseActions({ purchase }: PurchaseActionsProps) {
  ...
  const [removingSaleRecord, setRemovingSaleRecord] = useState(false)
  ...
  const onDeletePurchase = async () => {
    const confirmed = window.confirm(`Tem certeza que deseja apagar a compra \"${purchase.name}\"?`)
    if (!confirmed) return
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Não foi possível apagar a compra")
      }
  ...
  const onDeleteSaleRecord = async () => {
    const confirmed = window.confirm(`Deseja remover o registro de venda de \"${purchase.name}\"?`)
    if (!confirmed) return

    try {
      setRemovingSaleRecord(true)
      const response = await fetch(`/api/purchases/${purchase.id}/sell`, {
        method: "DELETE",
      })

      if (!response.ok) {
        throw new Error("Não foi possível remover o registro de venda")
      }
    ...
    return (
      <div className="flex flex-col items-end gap-2 md:flex-row md:items-center">
        ...
        <Button
          variant="outline"
          size="sm"
          onClick={onDeleteSaleRecord}
          disabled={removingSaleRecord}
